...
export default function updateCourier (services: IService): TCallbackFunction {
  return async (ctx: IContext): Promise<void> => {
    ...
    const errors = await Promise.all([
      updateCourierHelper(services, userData, id, EUserRow.name),
      updateCourierHelper(services, userData, id, EUserRow.email),
      updateCourierHelper(services, userData, id, EUserRow.password)
    ])
